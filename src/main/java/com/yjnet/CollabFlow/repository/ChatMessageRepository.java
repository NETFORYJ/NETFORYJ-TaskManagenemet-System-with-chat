...
public interface ChatMessageRepository extends JpaRepository<ChatMessage, Long> {
    ...
    List<ChatMessage> findByTenantIdAndIsGroupTrueOrderByCreatedAtAsc(Long tenantId);
    
    @Query("SELECT c FROM ChatMessage c WHERE (c.senderId = :user1 AND c.receiverId = :user2) OR (c.senderId = :user2 AND c.receiverId = :user1) ORDER BY c.createdAt ASC")
